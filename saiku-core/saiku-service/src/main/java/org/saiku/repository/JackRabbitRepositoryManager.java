...
public class JackRabbitRepositoryManager implements IRepositoryManager {
    ...
    public static synchronized JackRabbitRepositoryManager getJackRabbitRepositoryManager(String config, String data,
            String password, String oldpassword, String defaultRole) {
        if (ref == null)
    ...
    private List<IRepositoryObject> getRepoObjects(Node files, List<String> fileType, String username,
            List<String> roles, boolean includeparent) {
            ...
            if (includeparent) {
                ...
                if (files.getPrimaryNodeType().getName().equals("nt:file")) {
                    if (fileType != null && !filename.contains(FilenameUtils.getExtension(filename))) {
                        ...
                        repoObjects.add(new RepositoryFileObject(filename, "#" + files.getPath(), extension,
                                files.getPath(), acls));
                    }
                    if (files.getPrimaryNodeType().getName().equals("nt:folder")) {
                        ...
                        repoObjects
                                .add(new RepositoryFolderObject(files.getName(), "#" + files.getPath(), files.getPath(),
                                        acls, getRepoObjects(files, fileType, username, roles, false)));
                    }
                }
            } else {
                n = files.getNodes();
                        ...
                        if (acl2.canRead(node, username, roles)) {
                            ...
                            if (node.getPrimaryNodeType().getName().equals("nt:file")) {
                                if (fileType != null && !fileType
                                        .contains(FilenameUtils.getExtension(node.getName()))) {
                                    continue;
                                }
                                String extension = FilenameUtils.getExtension(nodename);

                                repoObjects
                                        .add(new RepositoryFileObject(node.getName(), "#" + node.getPath(), extension,
                                                node.getPath(), acls));
                            }
                            if (node.getPrimaryNodeType().getName().equals("nt:folder")) {
                                repoObjects.add(new RepositoryFolderObject(node.getName(), "#" + node.getPath(),
                                        node.getPath(), acls, getRepoObjects(node, fileType, username, roles, false)));
                            }
                            ...
                            if (!file.getName().startsWith("jcr:") && !file.getName().startsWith("rep:")) {
                                ...
                                if (acl2.canRead(file, username, roles)) {
                                    ...
                                    String relativePath = file
                                            .getPath();//repo.getName().getRelativeName(file.getName());
                                    ...
                                    if (file.getPrimaryNodeType().getName().equals("nt:saikufiles")) {
                                        ...
                                        repoObjects
                                                .add(new RepositoryFileObject(filename, "#" + relativePath, extension,
                                                        relativePath, acls));
                                    }
